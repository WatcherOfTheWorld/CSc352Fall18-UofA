 ...
 */
void addString(StringList *head, char *string){
    while(head->next != NULL){
        head = head->next;
    }
    ...
    new->next =NULL;
    head->next = new;
    new->this = strdup(string);

}

/*
 * split a string into string list
 */
StringList* splitString(char *string, char split){
